...
type MfaRow = { userId:string; secretEncrypted:string|null; enabled:boolean; confirmedAt:Date|null; failedAttempts:number; lockedUntil:Date|null; lastTotpStep:bigint|null };
...
export async function getMfaRow(userId:string):Promise<MfaRow|null> {
  const rows=await prisma.$queryRaw<MfaRow[]>`SELECT "userId","secretEncrypted","enabled","confirmedAt","failedAttempts","lockedUntil","lastTotpStep" FROM "UserMfa" WHERE "userId"=${userId} LIMIT 1`;
  return rows[0]??null;
}

export async function savePendingMfaSecret(userId:string,encryptedSecret:string):Promise<void> {
  await prisma.$executeRaw`INSERT INTO "UserMfa" ("userId","secretEncrypted","enabled","confirmedAt","failedAttempts","lockedUntil","lastTotpStep","createdAt","updatedAt") VALUES (${userId},${encryptedSecret},FALSE,NULL,0,NULL,NULL,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP) ON CONFLICT ("userId") DO UPDATE SET "secretEncrypted"=EXCLUDED."secretEncrypted","enabled"=FALSE,"confirmedAt"=NULL,"failedAttempts"=0,"lockedUntil"=NULL,"lastTotpStep"=NULL,"updatedAt"=CURRENT_TIMESTAMP`;
}

export async function verifyPendingMfaSetup(userId:string,code:string):Promise<{ok:boolean;recoveryCodes:string[]}> {
  return prisma.$transaction(async tx=>{
    const rows=await tx.$queryRaw<MfaRow[]>`SELECT "userId","secretEncrypted","enabled","confirmedAt","failedAttempts","lockedUntil","lastTotpStep" FROM "UserMfa" WHERE "userId"=${userId} FOR UPDATE`;
    const row=rows[0];
    ...
    await tx.$executeRaw`UPDATE "UserMfa" SET "enabled"=TRUE,"confirmedAt"=CURRENT_TIMESTAMP,"failedAttempts"=0,"lockedUntil"=NULL,"lastTotpStep"=NULL,"updatedAt"=CURRENT_TIMESTAMP WHERE "userId"=${userId}`;
