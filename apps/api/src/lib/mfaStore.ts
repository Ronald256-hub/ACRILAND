...
type MfaRow = { userId:string; secretEncrypted:string|null; enabled:boolean; confirmedAt:Date|null; failedAttempts:number; lockedUntil:Date|null };
...
export async function getMfaRow(userId:string):Promise<MfaRow|null> {
  const rows=await prisma.$queryRaw<MfaRow[]>`SELECT "userId","secretEncrypted","enabled","confirmedAt","failedAttempts","lockedUntil" FROM "UserMfa" WHERE "userId"=${userId} LIMIT 1`;
  return rows[0]??null;
}

export async function savePendingMfaSecret(userId:string,encryptedSecret:string):Promise<void> {
  await prisma.$executeRaw`INSERT INTO "UserMfa" ("userId","secretEncrypted","enabled","confirmedAt","failedAttempts","lockedUntil","createdAt","updatedAt") VALUES (${userId},${encryptedSecret},FALSE,NULL,0,NULL,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP) ON CONFLICT ("userId") DO UPDATE SET "secretEncrypted"=EXCLUDED."secretEncrypted","enabled"=FALSE,"confirmedAt"=NULL,"failedAttempts"=0,"lockedUntil"=NULL,"updatedAt"=CURRENT_TIMESTAMP`;
}

export async function verifyPendingMfaSetup(userId:string,code:string):Promise<{ok:boolean;recoveryCodes:string[]}> {
  return prisma.$transaction(async tx=>{
    const rows=await tx.$queryRaw<MfaRow[]>`SELECT "userId","secretEncrypted","enabled","confirmedAt","failedAttempts","lockedUntil" FROM "UserMfa" WHERE "userId"=${userId} FOR UPDATE`;
    const row=rows[0];
    if(!row?.secretEncrypted || row.enabled || !verifyTotp(decryptMfaSecret(row.secretEncrypted),code)) return {ok:false,recoveryCodes:[]};
    const recoveryCodes=generateRecoveryCodes();
    await tx.$executeRaw`UPDATE "UserMfa" SET "enabled"=TRUE,"confirmedAt"=CURRENT_TIMESTAMP,"failedAttempts"=0,"lockedUntil"=NULL,"updatedAt"=CURRENT_TIMESTAMP WHERE "userId"=${userId}`;
...
export async function verifyAndRecordMfaAttempt(userId:string,code:string):Promise<{ok:boolean;locked:boolean;recovery:boolean}> {
  return prisma.$transaction(async tx=>{
    ...
    if(verifyTotp(decryptMfaSecret(row.secretEncrypted),code)) {
