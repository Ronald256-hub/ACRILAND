...
departmentsRouter.post("/",requirePermission(PERMISSIONS.DEPARTMENT_MANAGE),async(req,res)=>{
  ...
  const row=await prisma.department.create({data:{organizationId:req.auth!.organizationId,code:input.code.toUpperCase(),name:input.name,costCentre:input.costCentre,branchId:input.branchId}});
