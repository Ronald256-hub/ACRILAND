...
geofencesRouter.post("/evaluate",requirePermission(PERMISSIONS.GEOFENCE_MANAGE),async(req,res)=>{
  ...
  for(const snapshot of latest.values()){
    ...
    for(const fence of fences){
      ...
      const previous=await prisma.geofenceState.findUnique({where:{geofenceId_vehicleId:{geofenceId:fence.id,vehicleId:vehicle.id}}});const changed=previous!==null&&previous.isInside!==inside;const now=snapshot.recordedAt;const policyBreach=(fence.policy==="RESTRICTED"&&inside)||(fence.policy==="ALLOWED"&&!inside);const breachEvidence=policyBreach&&(changed||previous===null);
      ...
      if(changed){
        ...
        transitions++;
        if(!policyBreach&&((inside&&fence.alertOnEntry)||(!inside&&fence.alertOnExit))){await prisma.operationalAlert.upsert({where:{organizationId_sourceType_sourceId_category:{organizationId,sourceType:"GEOFENCE_EVENT",sourceId:`${fence.id}:${snapshot.id}`,category:"GEOFENCE_TRANSITION"}},update:{status:"OPEN",message:`${vehicle.registrationNumber} ${inside?"entered":"exited"} ${fence.name}.`,closedAt:null,closedByUserId:null},create:{organizationId,category:"GEOFENCE_TRANSITION",severity:"INFO",sourceType:"GEOFENCE_EVENT",sourceId:`${fence.id}:${snapshot.id}`,title:`Geofence ${inside?"entry":"exit"} · ${vehicle.registrationNumber}`,message:`${vehicle.registrationNumber} ${inside?"entered":"exited"} ${fence.name}.`,vehicleId:vehicle.id}});}
      }
      if(breachEvidence){breaches++;const movement=previous===null?`was first observed ${inside?"inside":"outside"}`:`${inside?"entered":"exited"}`;const sourceId=`${fence.id}:${snapshot.id}`;const description=`${vehicle.registrationNumber} ${movement} ${fence.name} (${fence.policy.toLowerCase()} zone).`;await prisma.driverSafetyEvent.upsert({where:{organizationId_sourceType_sourceId_type:{organizationId,sourceType:"GEOFENCE_EVIDENCE",sourceId,type:"GEOFENCE_BREACH"}},update:{description,occurredAt:now},create:{organizationId,driverId:assignment?.driverId??null,vehicleId:vehicle.id,type:"GEOFENCE_BREACH",penaltyPoints:10,sourceType:"GEOFENCE_EVIDENCE",sourceId,description,occurredAt:now}});await prisma.operationalAlert.upsert({where:{organizationId_sourceType_sourceId_category:{organizationId,sourceType:"GEOFENCE_EVIDENCE",sourceId,category:"GEOFENCE_BREACH"}},update:{severity:"WARNING",status:"OPEN",title:`Geofence breach · ${vehicle.registrationNumber}`,message:description,vehicleId:vehicle.id,closedAt:null,closedByUserId:null},create:{organizationId,category:"GEOFENCE_BREACH",severity:"WARNING",sourceType:"GEOFENCE_EVIDENCE",sourceId,title:`Geofence breach · ${vehicle.registrationNumber}`,message:description,vehicleId:vehicle.id}});}
      if(inside&&fence.speedLimitKph!==null&&snapshot.speedKph!==null){const speed=Number(snapshot.speedKph),points=speedPenalty(speed,fence.speedLimitKph);if(points>0){speeding++;const sourceId=`${fence.id}:${snapshot.id}`;const description=`${vehicle.registrationNumber} recorded ${speed.toFixed(1)} km/h in ${fence.name}; limit ${fence.speedLimitKph} km/h.`;await prisma.driverSafetyEvent.upsert({where:{organizationId_sourceType_sourceId_type:{organizationId,sourceType:"TELEMETRY",sourceId,type:"SPEEDING"}},update:{penaltyPoints:points,description,occurredAt:snapshot.recordedAt},create:{organizationId,driverId:assignment?.driverId??null,vehicleId:vehicle.id,type:"SPEEDING",penaltyPoints:points,sourceType:"TELEMETRY",sourceId,description,occurredAt:snapshot.recordedAt}});if(speed-fence.speedLimitKph>=20)await prisma.operationalAlert.upsert({where:{organizationId_sourceType_sourceId_category:{organizationId,sourceType:"TELEMETRY",sourceId,category:"SPEEDING"}},update:{severity:"WARNING",status:"OPEN",message:description,vehicleId:vehicle.id,closedAt:null,closedByUserId:null},create:{organizationId,category:"SPEEDING",severity:"WARNING",sourceType:"TELEMETRY",sourceId,title:`Speed exception · ${vehicle.registrationNumber}`,message:description,vehicleId:vehicle.id}});}}
    }
  }
