...
geofencesRouter.post("/evaluate",requirePermission(PERMISSIONS.GEOFENCE_MANAGE),async(req,res)=>{
  ...
  for(const snapshot of latest.values()){
    ...
    for(const fence of fences){
      ...
      await prisma.geofenceState.upsert({where:{geofenceId_vehicleId:{geofenceId:fence.id,vehicleId:vehicle.id}},update:{isInside,lastEvaluatedAt:now,...(changed?{lastChangedAt:now}:{})},create:{organizationId,geofenceId:fence.id,vehicleId:vehicle.id,isInside,lastEvaluatedAt:now,lastChangedAt:now}});
      if(changed){
