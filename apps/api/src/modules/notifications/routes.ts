...
notificationsRouter.put("/preferences/me",async(req,res)=>{const input=z.object({emailEnabled:z.boolean(),criticalOnly:z.boolean().default(true),categories:z.array(z.string().min(1).max(100)).max(50).nullable().optional()}).parse(req.body);const categoryValue=input.categories===null?Prisma.JsonNull:input.categories;const updated=await prisma.notificationPreference.upsert({where:{userId:req.auth!.userId},update:{emailEnabled:input.emailEnabled,criticalOnly:input.criticalOnly,...(categoryValue!==undefined?{categories:categoryValue}:{})},create:{organizationId:req.auth!.organizationId,userId:req.auth!.userId,emailEnabled:input.emailEnabled,criticalOnly:input.criticalOnly,...(categoryValue!==undefined?{categories:categoryValue}:{})}});await audit(req,{action:"UPDATE",recordType:"NOTIFICATION_PREFERENCE",recordId:updated.id,newValue:{emailEnabled:updated.emailEnabled,criticalOnly:updated.criticalOnly,categories:updated.categories}});return res.json(updated);});
