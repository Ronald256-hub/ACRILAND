...
workspaceRouter.get("/search", async (req, res) => {
  ...
  if (p.has(PERMISSIONS.VEHICLE_VIEW)) {
    const rows = await prisma.vehicle.findMany({ where: { organizationId, archivedAt: null, OR: [
      { registrationNumber: { contains: q, mode: "insensitive" } }, { fleetNumber: { contains: q, mode: "insensitive" } },
      { vin: { contains: q, mode: "insensitive" } }, { make: { contains: q, mode: "insensitive" } }, { model: { contains: q, mode: "insensitive" } }
  ...
  if (p.has(PERMISSIONS.DRIVER_VIEW)) {
    const rows = await prisma.driver.findMany({ where: { organizationId, archivedAt: null, OR: [
      { fullName: { contains: q, mode: "insensitive" } }, { employeeNumber: { contains: q, mode: "insensitive" } }, { licenceNumber: { contains: q, mode: "insensitive" } }
  ...
  if (p.has(PERMISSIONS.TRIP_VIEW)) {
    const rows = await prisma.trip.findMany({ where: { organizationId, OR: [
      { tripNumber: { contains: q, mode: "insensitive" } }, { purpose: { contains: q, mode: "insensitive" } },
      { origin: { contains: q, mode: "insensitive" } }, { destination: { contains: q, mode: "insensitive" } }
    ] }, select: { id: true, tripNumber: true, purpose: true, origin: true, destination: true, status: true }, take: 6, orderBy: { createdAt: "desc" } });
  ...
  if (p.has(PERMISSIONS.MAINTENANCE_VIEW)) {
    const rows = await prisma.maintenanceWorkOrder.findMany({ where: { organizationId, OR: [
      { workOrderNumber: { contains: q, mode: "insensitive" } }, { title: { contains: q, mode: "insensitive" } }, { category: { contains: q, mode: "insensitive" } }
    ] }, select: { id: true, workOrderNumber: true, title: true, status: true }, take: 6, orderBy: { openedAt: "desc" } });
  ...
  if (p.has(PERMISSIONS.INCIDENT_VIEW)) {
    const rows = await prisma.incidentReport.findMany({ where: { organizationId, OR: [
      { incidentNumber: { contains: q, mode: "insensitive" } }, { location: { contains: q, mode: "insensitive" } }, { description: { contains: q, mode: "insensitive" } }
    ] }, select: { id: true, incidentNumber: true, type: true, severity: true, status: true, location: true }, take: 6, orderBy: { occurredAt: "desc" } });
  ...
  if (p.has(PERMISSIONS.VENDOR_VIEW)) {
    const rows = await prisma.vendor.findMany({ where: { organizationId, OR: [
      { name: { contains: q, mode: "insensitive" } }, { code: { contains: q, mode: "insensitive" } }, { category: { contains: q, mode: "insensitive" } }
    ] }, select: { id: true, code: true, name: true, category: true, status: true }, take: 6, orderBy: { name: "asc" } });
...
workspaceRouter.get("/vehicles/:id", requirePermission(PERMISSIONS.VEHICLE_VIEW), async (req, res) => {
  ...
  const [assignment, trips, maintenance, fuel, compliance, tyres, pm, telemetry, incidents, dispatches, costs, disposal] = await Promise.all([
    prisma.vehicleAssignment.findFirst({ where: { organizationId, vehicleId: id, status: "ACTIVE" }, include: { driver: { select: { id: true, fullName: true, employeeNumber: true, status: true, licenceExpiry: true } } }, orderBy: { startAt: "desc" } }),
    prisma.trip.findMany({ where: { organizationId, vehicleId: id }, select: { id: true, tripNumber: true, purpose: true, origin: true, destination: true, status: true, requestedDeparture: true, actualDeparture: true, actualReturn: true, distanceKm: true, driver: { select: { id: true, fullName: true } } }, orderBy: { requestedDeparture: "desc" }, take: 12 }),
    prisma.maintenanceWorkOrder.findMany({ where: { organizationId, vehicleId: id }, orderBy: { openedAt: "desc" }, take: 12 }),
    prisma.fuelTransaction.findMany({ where: { organizationId, vehicleId: id }, orderBy: { requestedAt: "desc" }, take: 15 }),
    prisma.complianceDocument.findMany({ where: { organizationId, vehicleId: id }, orderBy: { expiryDate: "asc" } }),
    prisma.tyreAsset.findMany({ where: { organizationId, currentVehicleId: id, status: "FITTED" }, orderBy: { position: "asc" } }),
    prisma.preventiveMaintenancePlan.findMany({ where: { organizationId, vehicleId: id, status: "ACTIVE" }, orderBy: { nextDueAt: "asc" } }),
    prisma.telemetrySnapshot.findFirst({ where: { organizationId, vehicleId: id }, orderBy: { recordedAt: "desc" } }),
    prisma.incidentReport.findMany({ where: { organizationId, vehicleId: id }, orderBy: { occurredAt: "desc" }, take: 10 }),
    prisma.dispatchPlan.findMany({ where: { organizationId, vehicleId: id }, orderBy: { plannedDeparture: "desc" }, take: 10 }),
    prisma.vehicleCostEntry.findMany({ where: { organizationId, vehicleId: id }, orderBy: { occurredAt: "desc" }, take: 100 }),
    prisma.vehicleDisposal.findFirst({ where: { organizationId, vehicleId: id, status: { in: ["REQUESTED", "APPROVED"] } }, orderBy: { requestedAt: "desc" } })
  ]);
  const issuedFuel = await prisma.fuelTransaction.aggregate({ where: { organizationId, vehicleId: id, status: "ISSUED" }, _sum: { totalCost: true, issuedLitres: true } });
  const closedMaintenance = await prisma.maintenanceWorkOrder.aggregate({ where: { organizationId, vehicleId: id, status: "CLOSED" }, _sum: { totalCost: true } });
  ...
  return res.json({ vehicle, assignment, trips, maintenance, fuel, compliance, tyres, preventiveMaintenance: pm, telemetry, incidents, dispatches, costs, disposal, economics: { acquisition, fuelSpend, fuelLitres: num(issuedFuel._sum.issuedLitres), maintenanceSpend, explicitCost, tco, costPerKm: vehicle.currentOdometerKm > vehicle.initialOdometerKm ? tco / (vehicle.currentOdometerKm - vehicle.initialOdometerKm) : null } });
});
...
workspaceRouter.get("/drivers/:id", requirePermission(PERMISSIONS.DRIVER_VIEW), async (req, res) => {
  ...
  const [assignment, trips, score, safetyEvents, incidents, inspections, fuel, dispatches] = await Promise.all([
    prisma.vehicleAssignment.findFirst({ where: { organizationId, driverId: id, status: "ACTIVE" }, include: { vehicle: { select: { id: true, registrationNumber: true, fleetNumber: true, make: true, model: true, status: true, currentOdometerKm: true } } }, orderBy: { startAt: "desc" } }),
    prisma.trip.findMany({ where: { organizationId, driverId: id }, select: { id: true, tripNumber: true, purpose: true, origin: true, destination: true, status: true, requestedDeparture: true, actualDeparture: true, actualReturn: true, distanceKm: true }, orderBy: { requestedDeparture: "desc" }, take: 15 }),
    prisma.driverScoreSnapshot.findFirst({ where: { organizationId, driverId: id }, orderBy: { calculatedAt: "desc" } }),
    prisma.driverSafetyEvent.findMany({ where: { organizationId, driverId: id }, orderBy: { occurredAt: "desc" }, take: 20 }),
    prisma.incidentReport.findMany({ where: { organizationId, driverId: id }, orderBy: { occurredAt: "desc" }, take: 10 }),
    prisma.vehicleInspection.findMany({ where: { organizationId, driverId: id }, select: { id: true, type: true, status: true, odometerKm: true, completedAt: true, createdAt: true, vehicle: { select: { registrationNumber: true } } }, orderBy: { createdAt: "desc" }, take: 15 }),
