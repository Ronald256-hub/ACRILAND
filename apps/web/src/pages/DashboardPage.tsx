...
export function DashboardPage() {
  ...
  const derived = useMemo(() => {
    ...
    if (data.dispatchBlocked > 0) alerts.push({ tone: "warning", icon: "route", title: `${data.dispatchBlocked} dispatch plan${data.dispatchBlocked === 1 ? "" : "s"} blocked`, detail: "Readiness checks found a trip, vehicle or driver control exception before movement.", href: "/dispatch", tag: "Dispatch" });
    if (data.openGeofenceBreaches > 0) alerts.push({ tone: "warning", icon: "route", title: `${data.openGeofenceBreaches} open geofence breach${data.openGeofenceBreaches === 1 ? "" : "es"}`, detail: "Connected-fleet location policy exceptions require review and accountable follow-up.", href: "/routes-geofences", tag: "Geofence" });
    ...
    if (data.pendingDisposals > 0) alerts.push({ tone: "info", icon: "vehicle", title: `${data.pendingDisposals} disposal item${data.pendingDisposals === 1 ? "" : "s"} in control pipeline`, detail: "Requested or approved asset retirement remains incomplete and cannot bypass operational release checks.", href: "/lifecycle", tag: "Lifecycle" });
  ...
  if (!data || !derived) return <div className="loading-state-v2"><span className="loading-ring" /><b>Building the live operating picture…</b><span>Reading fleet, driver, trip, workshop, financial and connected-fleet controls.</span></div>;
  ...
  return <>
    ...
    <div className="kpi-grid-v2">
      ...
      <KpiCard label="Notification delivery" value={data.pendingNotifications} detail={`${data.failedNotifications} failed delivery${data.failedNotifications === 1 ? "" : "ies"}`} icon="bell" tone={data.failedNotifications > 0 ? "danger" : data.pendingNotifications > 0 ? "warning" : "success"} />
      ...
      <KpiCard label="Lifecycle control" value={data.pendingDisposals} detail={`${data.activeVendors} active vendor${data.activeVendors === 1 ? "" : "s"}`} icon="gauge" tone={data.pendingDisposals > 0 ? "warning" : "neutral"} />
    ...
    <div className="dashboard-grid-v2">
      ...
      <section className="panel-v2 span-8">
        ...
        <div className="panel-body-v2 workflow-grid-v2">
          ...
          <Link to="/notifications"><span className="workflow-icon"><Icon name="bell" /></span><div><span>Delivery queue</span><b>{data.pendingNotifications}</b><small>{data.failedNotifications} failed</small></div></Link>
        </div>
      </section>
