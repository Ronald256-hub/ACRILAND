...
export function DriversPage() {
  ...
  const changeStatus = async (driver: Driver, status: "ACTIVE" | "OFF_DUTY" | "SUSPENDED" | "INACTIVE") => {
    const reason = status === "ACTIVE" ? "Driver returned to active duty" : window.prompt(`Reason for setting ${driver.fullName} to ${status.replaceAll("_", " ")}:`);
    if (reason === null) return;
    setStatusBusy(true); setError("");
    try { await api(`/drivers/${driver.id}`, { method: "PATCH", body: JSON.stringify({ status, reason: reason || undefined }) }); await load(); }
    catch (e) { setError(e instanceof Error ? e.message : "Unable to change driver status."); }
    ...
    <section className="panel-v2">
      ...
      <div className={`fleet-register-layout-v2 ${selected ? "with-detail" : ""}`}>
        ...
        {selected && <aside className="vehicle-detail-v2 driver-detail-v2"><div className="detail-head-v2"><DriverAvatar name={selected.fullName} photoEndpoint={selected.photoEndpoint ?? null} size="medium" /><div><span>AUTHORIZED DRIVER</span><h3>{selected.fullName}</h3><p>{selected.employeeNumber}</p></div><button onClick={() => setSelected(null)} aria-label="Close driver detail"><Icon name="close" size={17} /></button></div>
          ...
          {me?.permissions.includes("driver.edit") && <div className="driver-lifecycle-v11"><span>DRIVER LIFECYCLE</span><div>{selected.status === "ACTIVE" ? <><button className="button-v2 secondary" disabled={statusBusy} onClick={() => void changeStatus(selected,"OFF_DUTY")}>Set off duty</button><button className="button-v2 secondary" disabled={statusBusy} onClick={() => void changeStatus(selected,"SUSPENDED")}>Suspend</button><button className="button-v2 danger" disabled={statusBusy} onClick={() => void changeStatus(selected,"INACTIVE")}>Remove from active roster</button></> : <button className="button-v2 primary" disabled={statusBusy} onClick={() => void changeStatus(selected,"ACTIVE")}>Return to active duty</button>}</div></div>}
        </aside>}
      </div>
    </section>
  </>;
